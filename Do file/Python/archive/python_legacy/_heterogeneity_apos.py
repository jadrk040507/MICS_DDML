...
def fit_base_apos(
    ...
    """
    cp_file = CHECKPOINT_DIR / (
        f"apos_het_{dataset_type}_{outcome_var}_{learner_name}_k{n_folds}_r1.pkl"
    )
...
def estimate_gate_apos(
    ...
    for i, col in enumerate(groups.columns):
        ...
        try:
            raw_key = int(float(raw))
        except Exception:  # noqa: BLE001
            raw_key = raw
        label = None
        if group_labels is not None:
            for k, v in group_labels.items():
                if k == raw_key or str(k) == raw or str(k) == str(raw_key):
                    label = v
                    break
        if label is None:
            label = str(raw_key)
...
def estimate_cate_apos_spline(
    ...
    formula = f"0 + bs(x, df={df_spline}, degree={degree}, include_intercept=True)"
