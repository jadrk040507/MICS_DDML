...
def _fit_named_apos(dt, outcome_var, confounder_groups, learner_name, n_folds, n_rep,
    ...
    cp_long = CHECKPOINT_DIR / (
        f"apos_sens_{dataset_type}_{outcome_var}_{learner_name}_k{n_folds}_r{n_rep}.pkl"
    )
...
def run_spec(dt, outcome, confounders, dataset_type, learner_name, n_folds, n_rep,
    ...
    bench = {}  # group -> benchmark arrays + reduced-specification RV/RVa arrays
    for group_name, gcols in group_map.items():
        ...
        try:
            cp_short = CHECKPOINT_DIR / (
                f"apos_sens_short_{dataset_type}_{out_var}_{learner_name}_"
                f"{group_name}_k{n_folds}_r{n_rep}.pkl"
            )
            ...
            gain = _contrast_gain(contrast, contrast_s, var_y)
            bench[group_name] = gain
        except Exception as exc:  # noqa: BLE001
...
def _fit_named_irm(dt, outcome_var, treatment_var, confounder_groups, learner_name,
    ...
    cp = CHECKPOINT_DIR / (
        f"irm_sens_{dataset_type}_{outcome_var}_{treatment_var}_{learner_name}_"
        f"k{n_folds}_r{n_rep}.pkl"
    )
