...
\end{center}
\begin{singlespace}
Household decisions to treat drinking water are shaped by numerous factors, making causal inference challenging in non-experimental settings. This paper estimates the causal effect of household water treatment on E. coli contamination\footnote{and the effect of contamination on child diarrhea} using Double/Debiased Machine Learning (DDML), a framework well suited for addressing high-dimensional confounding. The analysis draws on MICS data from \ToTSample{} households across 25 countries in Sub-Saharan Africa, Latin America, and Asia, incorporating objective measures of water quality. The DDML estimates refine the upward bias present in conventional OLS estimates while confirming a statistically significant causal effect of household water treatment in reducing E. coli contamination. The analysis also reveals substantial heterogeneity in treatment effects and in the magnitude of DDML correction: effects are larger in settings where source water is highly contaminated. Among households relying on highly contaminated source water, OLS estimates suggest that water treatment reduces the probability of highly contaminated drinking water by 21.2 percentage points. After applying DDML, the estimated effect is corrected to approximately 15 percentage points, representing nearly a 30 percent reduction relative to the OLS estimate.
...
While scientific evidence shows that household water treatment can reduce E. coli contamination under controlled laboratory conditions (CITE), its effectiveness in real-world settings may be substantially more limited, as correct use, consistent adherence, and safe post-treatment storage are often difficult to maintain. Effective use of chlorine and other treatment technologies requires correct dosing and consistent adherence to recommended procedures, which can be challenging for households. The appropriate dosage varies with baseline contamination, water turbidity, and storage conditions, and chlorine may lose potency when stored for extended periods. Even boiling does not fully ensure pathogen removal if treated water is later recontaminated during handling or storage \citep{wright2004household}. As a result, real-world effectiveness may fall short of laboratory efficacy estimates.

A meta-analysis combining 21 studies finds a substantial effect of household chlorination on water quality and child health outcomes \citep{arnold2007treating}. The results show that the probability of stored water containing any detectable E. coli contamination was reduced by 80 percent (pooled relative risk: 0.20), while the risk of child diarrhea declined by 29 percent (pooled relative risk: 0.71). However, two subsequent large-scale randomized controlled trials conducted in rural Bangladesh and Kenya found that chlorination tablet interventions did not significantly reduce diarrheal disease incidence or improve anthropometric outcomes \citep{pickering2019wash, luby2018effects}. These findings suggest that while water treatment can improve measured water quality, translating these improvements into sustained health gains remains challenging in practice. [Ask results to Amy].

\begin{figure}[htbp]\centering
\caption{Results from Meta-Analysis}\label{Desc_0}
\begin{subfigure}[b]{1\textwidth}\includegraphics[width=\textwidth]{ArnoldEcoli.png}\end{subfigure}
{\footnotesize  \begin{flushleft} Notes: .\end{flushleft}}
\end{figure}

Moreover, most causal evidence on household water treatment comes from experimental settings in which households are provided with new treatment products and, in some cases, complementary components such as education or behavior-change interventions. While these studies provide credible estimates of program impacts, the results may differ from non-experimental contexts, where households often use treatment methods inconsistently or incorrectly, and where compliance, monitoring, and supportive activities are more limited. As a result, treatment effects observed in observational settings may be smaller than those reported in controlled trials.
...
The DDML framework addresses the second and third challenges-regularization bias arising from the bias-variance tradeoff and the mismatch between predictive objectives and causal estimation-through orthogonalization and cross-fitting. The "double" in DDML refers to the orthogonalization of both the outcome and treatment variables with respect to high-dimensional controls, extending the Frisch-Waugh-Lovell theorem to flexible machine-learning settings. Cross-fitting further reduces overfitting and regularization bias by separating nuisance estimation from treatment effect estimation.

\begin{enumerate}
\item \textbf{Neyman orthogonalization.} This provides a robust way to define scores ($\phi_i$) such that small errors in specification do not contaminate the causal estimate. The most important application concerns deviations from the true parameters caused by regularization. Formally, Neyman orthogonality requires:
\begin{equation}
...
A commonly used moment condition is the AIPW (Augmented Inverse Probability Weighting) score function, which is doubly robust in the sense that the estimator remains consistent if either the outcome model or the propensity score model is correctly specified. Many AIPW score functions also satisfy Neyman orthogonality, making them robust to small estimation errors in nuisance parameters and well-suited for DDML estimation.

We employ the Interactive Regression Model (IRM) to capture the possibility of heterogeneous treatment effects across households. Unlike standard linear models that impose a constant treatment effect, the IRM allows the effect of treatment to vary flexibly with observed covariates:

\begin{equation}
Y = g_0(D, X) + \varepsilon,
\end{equation}

where the treatment effect is a function of the covariates X, referred to as the Conditional Average Treatment Effect (CATE), and may take either linear or nonlinear forms. Although the IRM permits treatment effects to vary across observations, the main parameter of interest reported in the analysis is the Average Treatment Effect (ATE), obtained by averaging the estimated conditional treatment effects over the empirical distribution of covariates.

DDML substantially improves adjustment for observed confounders by flexibly modeling nonlinearities, interactions, and high-dimensional relationships that may be misspecified in conventional OLS. As a result, it reduces bias arising from functional-form restrictions and incomplete control of observed covariates. However, DDML does not resolve omitted variable bias if important confounders remain entirely unobserved.

The key identifying assumption is conditional independence: conditional on the observed covariates $X_{i}$, the household's water-treatment decision is assumed to be independent of unobserved determinants of drinking-water contamination. In other words, after controlling for a rich set of household, environmental, and socioeconomic characteristics, treatment status is interpreted as being as good as randomly assigned with respect to latent contamination risk.

Concretely, for each outcome, we consider several learners: a linear model, penalized linear models and nonlinear tree-based methods. All learners are specified for both the outcome model and the treatment propensity model. In addition, we construct a \textit{stacked} ensemble learner: each base learner generates cross-validated out-of-fold predictions, which are combined via feature union and passed to a ridge regression meta-learner that selects optimal stacking weights by minimizing out-of-sample loss. Final stacking weights are reported as the normalized absolute coefficients of the ridge meta-learner.

% OLS estimates are excluded from specific-treatment panels due to quasi-complete separation: country fixed effects perfectly predict zero treatment in several countries (e.g., Benin and the Central African Republic have a 0\% boil rate), causing infinite MLE in the logistic propensity model and rank-deficient fits in the outcome model. Regularized learners (lasso, ridge, elastic net) are unaffected, as penalties prevent coefficient divergence. The stacked ensemble for specific treatments uses only five base learners (lasso, ridge, elastic net, random forest, XGBoost) to avoid OLS contamination. Cross-fitting is implemented with $K = 5$ folds and 2 repetitions.

% Is this still true after discussion of causal pathway?: An important feature of our specification concerns the inclusion of source-water E. coli controls. For diarrhea outcomes, source-water contamination dummies are included as confounders, since they plausibly affect both treatment adoption and child health. For E. coli contamination outcomes, however, source-water contamination is \textit{not} included: it lies on the causal pathway from treatment to the outcome (i.e., it is a mediator rather than a confounder), and conditioning on it would block the effect of interest.
...
\end{equation}
where $M_{\text{source}}$ is the baseline water quality measured as CFU \textit{E. coli} count, discretized as: 0 = No risk (0 CFU), 1 = Some risk ($1 \leq \text{CFU} < 100$), and 2 = Very high risk (CFU $\geq 100$). This discretization is motivated by both the measurement protocol and international health guidelines.

\subsubsection{Measurement Protocol and WHO Guidelines}

The MICS water quality module follows standardized protocols developed jointly by UNICEF and the Joint Monitoring Programme (JMP). Field teams collected 100 mL water samples from two distinct locations:

\begin{enumerate}
    \item \textbf{Source water ($M_{\text{source}}$)}: Water collected directly from the household's primary drinking water source (e.g., piped connection, well, spring, rainwater collection, or vendor). This represents the baseline contamination level before any household-level handling or treatment.
    
    \item \textbf{Drinking water ($M_{\text{drinking}}$)}: Water stored in the household and actually consumed, collected by requesting a glass of drinking water from the respondent. This reflects the cumulative effect of source quality, household treatment practices, transport, and storage conditions.
\end{enumerate}

Samples were analyzed using portable membrane filtration (CompactDry EC plates), with results reported as colony-forming units (CFU) per 100 mL. The measurement is right-censored at 100 CFU, as the test plates become unreadable beyond this threshold.

The World Health Organization (WHO) Guidelines for Drinking-water Quality state that \textit{E. coli} ``must not be detectable in any 100 mL sample'' of water intended for drinking \citep{who_gdwq_2017, who_gdwq_2022}. The WHO explicitly notes that ``the presence of \textit{E. coli} indicates faecal contamination of drinking-water'' and that ``immediate investigative action must be taken if \textit{E. coli} are detected.'' \citep{who_gdwq_2022}

While the WHO guideline is zero tolerance, empirical research in low- and middle-income countries recognizes that faecal contamination is widespread in rural and untreated water supplies \citep{who_gdwq_2022}. The three-category discretization used in this analysis aligns with risk-based frameworks:

\begin{itemize}
    \item \textbf{No risk (0 CFU)}: Meets WHO guideline; no detectable faecal contamination.
    \item \textbf{Some risk (1--99 CFU)}: Indicates faecal contamination requiring attention; associated with elevated risk of waterborne disease.
    \item \textbf{Very high risk ($\geq 100$ CFU)}: Indicates severe faecal contamination; substantially elevated risk of diarrheal disease and other waterborne illnesses.
\end{itemize}

Research indicates that \textit{E. coli} concentrations exceeding 100 CFU/100 mL are associated with significantly increased odds of detecting enteric pathogens including \textit{Campylobacter}, \textit{Cryptosporidium}, \textit{Salmonella}, and pathogenic \textit{E. coli} O157:H7 \citep{van_dyke_2012, banihashemi_2015}. This threshold thus represents a meaningful health risk breakpoint in addition to being the measurement ceiling.
...
An important identification insight concerns the role of $M_{\text{source}}$. If we ignore the path $M_{\text{source}} \to T$, we end up comparing ``clean-source'' households who do not treat with ``dirty-source'' households who do treat. Because those who treat water are systematically starting from a worse baseline, a simple comparison would underestimate the effectiveness of treatment, as $M_{\text{source}}$ acts as a confounder. \footnote{Conversely, conditioning on $M_{\text{drinking}}$ in the base model would block the biological pathway (it acts as a mediator), which would lead to observing no treatment effect. Our empirical strategy therefore controls for $M_{\text{source}}$ but does not condition on $M_{\text{drinking}}$.}
   $Home$ = drinking water E. coli; $Y$ = child diarrhea.
   \end{flushleft}
   }
   \end{figure}
...
\subsection{Characteristics associated with household water treatment (JA To update)}
To characterize the factors associated with household water treatment, this study employs

\subsection{Control Variables}
This study reports three sets of estimates: (1 and 2) OLS estimates with and without controls, and (3) Double/Debiased Machine Learning (DDML) estimates using the same set of control variables as in the OLS specification.

The control vector includes the following household and environmental characteristics. Household wealth is captured by a set of five wealth-index quintile dummies. The education of the household head is represented by five dummies corresponding to none, primary, lower secondary, upper secondary, and college education, plus a category for missing values.

For environmental factors, urban residence is indicated by a binary variable. Water source type is captured by 15 detailed dummies corresponding to the MICS classification: piped water into the dwelling, piped water to yard/plot, piped water to neighbor, public tap/standpipe, tube well/borehole, protected well, unprotected well, protected spring, unprotected spring, rainwater, tanker-truck, cart with small tank, surface water, bottled water, and other. Sanitation is captured by a binary indicator for improved latrine facilities. The number of children in the household (0, 1, or 2) is included to account for differences in exposure, water use, and hygiene practices.

Importantly, country fixed effects (25 dummies) are included to account for cross-country heterogeneity in baseline contamination risk, infrastructure quality, climate, and public health environments. Including country indicators in the covariate set also allows the IRM framework to capture potential differences in treatment effects across country contexts, as the effectiveness of household water treatment may vary with local environmental and institutional conditions.
...
\input{Table/Descript_ExtMain_Risk_ext.tex}
}
%-------------------------------------------------------------%
...
Before presenting causal estimates, we first examine the non-causal associations between household characteristics and water treatment adoption. This descriptive analysis serves three purposes: (1) it validates the hypothesized causal chain by showing that source water contamination predicts treatment adoption; (2) it identifies which household characteristics are most strongly associated with treatment behavior; and (3) it provides insight into potential confounding patterns that the DDML framework must address.

We employ two complementary machine learning approaches: (1) LASSO-logistic regression for interpretable coefficients and average marginal effects (AMEs), and (2) XGBoost (extreme gradient boosting) for flexible modeling of non-linearities and interactions, with variable importance measures to identify the most predictive characteristics.

\subsubsection{LASSO-Logistic Regression}

We estimate a sparse logistic regression model with L1 regularization:
\begin{equation}
\max_{\beta} \sum_{i=1}^{n} \left[ D_i \log(p_i) + (1-D_i)\log(1-p_i) \right] - \lambda \sum_{j=1}^{p} |\beta_j|,
\end{equation}
where $D_i$ is the treatment indicator, $p_i = \Pr(D_i=1|X_i)$, and $\lambda$ is selected via 10-fold cross-validation to minimize out-of-sample deviance. The LASSO penalty shrinks coefficients of less predictive covariates toward zero, providing automatic variable selection in high-dimensional settings.

We estimate two specifications:
\begin{enumerate}
    \item \textbf{Any treatment}: Binary outcome (any treatment vs. no treatment)
    \item \textbf{Multitreatment}: Multinomial outcome (boil, chlorine, filter, other vs. no treatment)
\end{enumerate}

%-------------------------------------------------------------%
{\renewcommand\normalsize{\small}\normalsize\singlespacing
\input{Table/LASSO_AnyTreatment.tex}
}
%-------------------------------------------------------------%

Table \ref{tab:lasso_any} presents the LASSO-logistic regression results for any treatment. After variable selection, the model retains [X] out of [Y] covariates. Key findings include:

\begin{itemize}
    \item \textbf{Source water contamination}: Households with moderate-risk source water (1--100 CFU) are [X] percentage points more likely to treat compared to those with no contamination (p\textless0.01). For very high-risk sources ($\geq$100 CFU), the probability increases by [Y] percentage points. This confirms the hypothesized causal pathway $M_{\text{source}} \to T$ and validates that households respond to contamination risk.
    
    \item \textbf{Wealth}: Higher wealth quintiles show monotonically increasing treatment probabilities, with quintile 5 households [X]pp more likely to treat than quintile 1 (p\textless0.01). This pattern is consistent with treatment requiring financial resources for fuel, chlorine, or filters.
    
    \item \textbf{Education}: Household head education shows a positive gradient, with college-educated heads [X]pp more likely to treat compared to no education (p\textless0.05). This may reflect greater health knowledge or risk awareness.
    
    \item \textbf{Urban residence}: Urban households are [X]pp more likely to treat (p\textless0.01), potentially due to better access to treatment products and infrastructure.
    
    \item \textbf{Country fixed effects}: Large cross-country variation persists even after controlling for observables, ranging from [X]\% treatment prevalence in [Country A] to [Y]\% in [Country B]. This reflects differences in infrastructure, cultural practices, and public health environments.
\end{itemize}

%-------------------------------------------------------------%
{\renewcommand\normalsize{\small}\normalsize\singlespacing
\input{Table/LASSO_MultiTreatment.tex}
}
%-------------------------------------------------------------%

Table \ref{tab:lasso_multi} shows multinomial LASSO results by treatment type. Notably:
\begin{itemize}
    \item \textbf{Boiling}: Most strongly associated with education and wealth; less common in urban areas (potentially due to fuel costs or alternative options).
    \item \textbf{Chlorine}: More common among wealthier households and in urban areas; strongly predicted by source contamination.
    \item \textbf{Filters}: Associated with higher wealth quintiles; may reflect higher upfront costs.
    \item \textbf{Other methods}: More common in rural areas and among poorer households.
\end{itemize}

\subsubsection{XGBoost Variable Importance}

To complement the parametric LASSO approach and capture potential non-linearities and interactions, we estimate an XGBoost classifier. XGBoost is a tree-based ensemble method that builds sequential decision trees, with each tree correcting errors from the previous iteration \citep{chen_xgboost_2016}. The method excels at capturing complex interactions without explicit specification.

We use the following hyperparameters (selected via grid search): maximum depth = [X], learning rate ($\eta$) = [Y], number of rounds = [Z], subsample ratio = [W]. The model achieves an out-of-sample AUC of [AUC], compared to [AUC] for LASSO-logistic, suggesting [comparable/superior] predictive performance.

%-------------------------------------------------------------%
{\renewcommand\normalsize{\small}\normalsize\singlespacing
\input{Table/XGBoost_VariableImportance.tex}
}
%-------------------------------------------------------------%

Table \ref{tab:xgb_importance} reports variable importance measured by \textit{gain}, which captures the average reduction in the loss function (logistic deviance) attributable to splits on each variable. Higher gain indicates greater predictive importance.

The top predictors of water treatment are:
\begin{enumerate}
    \item \textbf{Source water risk level} (gain: [X]\%): Confirms that baseline contamination is the strongest predictor of treatment, validating the causal chain.
    \item \textbf{Country fixed effects} (gain: [Y]\%): Substantial cross-country heterogeneity in treatment practices.
    \item \textbf{Wealth index} (gain: [Z]\%): Economic resources constrain treatment adoption.
    \item \textbf{Water source type} (gain: [W]\%): Households using different source types (piped, well, surface water) face different contamination risks and treatment needs.
    \item \textbf{Urban/rural} (gain: [V]\%): Reflects infrastructure and market access differences.
\end{enumerate}

Notably, child characteristics (age, sex) and sanitation show relatively low importance for treatment prediction, suggesting these variables primarily affect health outcomes rather than treatment decisions directly.

\subsubsection{Comparison with OLS}\label{OLS}

For comparison, Table \ref{tab:ols_treatment} presents linear probability model (OLS) estimates of the same specifications. The OLS results show similar directional patterns but differ in magnitude for several covariates, particularly for extreme values of source contamination and wealth. The machine learning methods better capture:
\begin{itemize}
    \item Non-linear wealth gradients (treatment probability plateaus at higher wealth levels)
    \item Interactions between source risk and wealth (poor households with highly contaminated sources show lower treatment rates than rich households in similar circumstances)
    \item Country-specific patterns that deviate from the linear average
\end{itemize}

These descriptive results validate the hypothesized causal chain and highlight the importance of flexibly controlling for source water contamination and household characteristics. The DDML framework builds on these insights by using machine learning to adjust for confounding while providing unbiased causal estimates of treatment effects on water quality and health outcomes.
...
\input{Table/benchmark_effects}

\input{Table/heterogeneity_risksource}


\begin{landscape}
%\include{Table/SomeRiskHome_PLM_BASE_EXT_All}\label{SR_plm}
\end{landscape}

\begin{landscape}
%\include{Table/VeryHighRiskHome_PLM_BASE_EXT_All}\label{VH_plm}
\end{landscape}
...
%-------------------------------------------------------------%
{\renewcommand\normalsize{\small}\normalsize\singlespacing
\input{Table/table_diarrhea.tex}
}
%-------------------------------------------------------------%

%-------------------------------------------------------------%
{\renewcommand\normalsize{\small}\normalsize\singlespacing
\input{Table/table_diarrhea_subgroups.tex}
}
%-------------------------------------------------------------%
